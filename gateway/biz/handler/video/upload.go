...
func UploadChunkHandler(ctx context.Context, c *app.RequestContext) {
	...
	if uploadID == "" {
		c.JSON(consts.StatusBadRequest, map[string]interface{}{"code": 400, "msg": "upload_id 不能为空"})
		return
	}
	...
	if len(chunkData) < minChunkSize {
		c.JSON(consts.StatusBadRequest, map[string]interface{}{"code": 400, "msg": "分片大小不能小于 5MB"})
		return
	}
