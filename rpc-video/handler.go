...
func (s *VideoServiceImpl) MergeFile(ctx context.Context, req *video.MergeFileReq) (resp *video.MergeFileResp, err error) {
	...
	if err != nil {
		log.Printf("[MergeFile] Redis 不可用，降级继续合并: %v", err)
		finishedChunks = nil
	}

	if finishedChunks != nil && int32(len(finishedChunks)) < req.TotalChunks {
	...
	// 4.1 尝试上传到 MinIO（失败则保留本地URL）
	objectName := fmt.Sprintf("videos/%s/%s", userID, req.FileHash+"_"+req.Filename)
	if minioURL, upErr := commonMinio.UploadFile(ctx, objectName, filePath, "video/mp4"); upErr == nil {
		fileURL = minioURL
	} else {
		log.Printf("[MergeFile] MinIO 上传失败，继续使用本地URL: %v", upErr)
	}
...
func (s *VideoServiceImpl) Transcode(ctx context.Context, req *video.TranscodeReq) (resp *video.TranscodeResp, err error) {
	...
	// 检查文件是否存在（先精确匹配，再宽松查找）
	file, err := db.GetFileByHashAndUser(req.FileHash, userID)
	if err != nil || file == nil {
		log.Printf("[Transcode] 精确查找失败(userID=%s), 尝试按 hash 回退查找", userID)
		file, err = db.GetFileByHash(req.FileHash)
		if err != nil || file == nil {
			log.Printf("[Transcode] 回退查找也失败: hash=%s, err=%v", req.FileHash, err)
			resp.Code = 404
			resp.Msg = "文件不存在"
			return resp, nil
		}
		log.Printf("[Transcode] 回退查找成功: file.UserID=%s", file.UserID)
	}
	...
	// 发布转码任务事件到 Kafka（用于 videoTranscode 服务消费）
	if pubErr := transcode.PublishTaskEvent(ctx, taskID, req.FileHash, userID, req.Resolutions); pubErr != nil {
		log.Printf("[Transcode] Kafka 发布失败(不影响主流程): %v", pubErr)
	}
